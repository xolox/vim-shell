...
function! xolox#shell#misc#open#file(path, ...) " {{{1
  ...
  "
  "     :call xolox#shell#misc#open#file('/path/to/my/file', 'firefox', 'google-chrome')
  "
  ...
  if xolox#shell#misc#os#is_win()
    try
      ...
      silent execute printf(command, xolox#shell#misc#escape#shell(a:path))
    endtry
    return
  elseif has('macunix')
    ...
    for handler in s:handlers + a:000
      if executable(handler)
        call xolox#shell#misc#msg#debug("open.vim %s: Using '%s' to open '%s'.", s:version, handler, a:path)
  ...
  throw printf(s:enoimpl, s:script, 'xolox#shell#misc#open#file')
endfunction

function! xolox#shell#misc#open#url(url) " {{{1
  ...
  call xolox#shell#misc#open#file(url, 'firefox', 'google-chrome')
endfunction
